...
// custom login styles
  	...
  	if('enable' === $styles)
  {
  		...
  		wp_enqueue_style('myplugin',plugin_dir_url(dirname(__FILE__)).'public/css/firstplugin-log.css',array(),null,'screen');
  		wp_enqueue_script('myplugin',plugin_dir_url(dirname(__FILE__)).'public/js/firstplugin-log.js',array(),null,true);
  		}
  }
...
//admin color scheme
  function myplugin_custom_admin_scheme($user_id)
  {
  	$scheme = 'default';
  	$options = get_option('myplugin_options',myplugin_options_default());
  	if (isset( $soptions['custom_scheme'])&&!empty( $options['custom_scheme']))
    {
